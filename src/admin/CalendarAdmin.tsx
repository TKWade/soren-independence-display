...
import { setCalendarSelection, requestCalendarAction, connectGoogleCalendar, disconnectGoogleCalendar } from '../data/calendarRepository'
...
export function CalendarAdmin({data,run}:{data:HouseholdData;run:RunAction}) {
 ...
 const [connectionNotice,setConnectionNotice]=useState(()=>new URLSearchParams(window.location.search).get('googleCalendar')==='connected'?'Google Calendar connected. Choose calendars below.':new URLSearchParams(window.location.search).get('googleCalendar')==='failed'?'Google connection was not completed. Try Connect Google again and grant both read-only permissions.':'')
 ...
 <p>Connect Google, enable a calendar and choose Include and evaluate events, then use Sync Now. Review new events in Calendar inbox.</p>{connectionNotice&&<p role="status">{connectionNotice}</p>}
 <button onClick={()=>void run(()=>connectGoogleCalendar(data.household.id))}>Connect Google</button><button disabled>Connect Microsoft (coming next)</button>
 ...
 {connection.provider==='google'&&<button disabled={connection.status==='disabled'} onClick={()=>void run(async()=>setConnectionNotice(await disconnectGoogleCalendar(data.household.id,connection.id)))}>Disconnect Google</button>}
